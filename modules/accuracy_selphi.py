...
class DataIngestion:
    def __init__(
        self, filepath: str, chunk_size: int = 10_000, samples_file: str = ""
    ) -> None:
    ...
    def initial_check(self, vcf_path: str) -> str:
        ...
        if not (os.path.exists(vcf_path + ".tbi") or os.path.exists(vcf_path + ".csi")):
            ...
            subprocess.run(
                f"bcftools index {vcf_path} --threads {os.cpu_count()}", shell=True
            )
        return vcf_path
    ...
    def _ingest_data(self, vcf_path: str, chrom: str, start: int, end: int):
        ...
        def _load_haplotypes(args):
            ...
            if self.samples_file != "":
                commands = [
                    (
                        f"bcftools view -r {chrom}:{start}-{end} --regions-overlap 1 "
...
class CalculateMetrics:
    def __init__(
        self,
        imputed: sparse.csc_matrix,
        wgs: sparse.csc_matrix,
        vID_imputed: List[str],
        vID_wgs: List[str],
        sID_imputed: List[str],
        sID_wgs: List[str],
        chunk_index: int,
        ...
        for axis in [0, 1]:
            ...
            if axis == 1:
                ...
                f1 = np.round(
                    2
                    * np.divide(
                        np.multiply(precision, recall), np.add(precision, recall)
                    ),
                    decimals=6,
                )
                ...
                metrics = [TP, FP, FN, TN]
                metrics = [
                    np.sum(m.reshape(reshaped_length, 2), axis=1) for m in metrics
                ]
    ...
    def accuracy(self) -> Dict[str, np.ndarray]:
        ...
        correct_pred_per_sample = np.divide(
            correct_pred_sum.reshape(-1, 2).sum(axis=1), 2.0
        )
    ...
    def compute_IQS(self) -> dict:
        ...
        x = np.array([dose_to_probability[d] for d in imputed.ravel()]).reshape(
            imputed.shape[0], -1
        )
        y = np.array([dose_to_probability[d] for d in wgs.ravel()]).reshape(
            wgs.shape[0], -1
        )
    ...
    def r2(self) -> Dict[str, np.ndarray]:
        ...
        den_var = (
            np.sqrt(
                (N * x_squared_sum_var - x_sum_var**2)
                * (N * y_squared_sum_var - y_sum_var**2)
            )
            + 1e-15
        )
        ...
        num_sample = (
            len(imputed) * xy_sum_sample - x_sum_sample * y_sum_sample
        ) + 1e-15
        den_sample = (
            np.sqrt(
                (len(imputed) * x_squared_sum_sample - x_sum_sample**2)
                * (len(imputed) * y_squared_sum_sample - y_sum_sample**2)
            )
            + 1e-15
        )
    ...
    def _get_sorted_csv_files(self, suffix: str):
        ...
        csv_files = [
            os.path.join(self.unique_folder_name, file)
            for file in os.listdir(self.unique_folder_name)
            if file.endswith(suffix)
        ]
        return self.custom_sort(csv_files)

    def concatenate_files(self) -> str:
        fn_per_sample = os.path.join(
            self.unique_folder_name,
            f"saved_chunked_tmp_{self.chunk_index}_x_sample.csv",
        )
        fn_per_variant = os.path.join(
            self.unique_folder_name,
            f"saved_chunked_tmp_{self.chunk_index}_x_variant.csv",
        )

        # Concatenate variant files
        csv_files_variant = self._get_sorted_csv_files(
            f"_res_to_join_{self.chunk_index}_x_variant.csv"
        )
        cmd_variant = f"paste -d ',' {' '.join(csv_files_variant)} > {fn_per_variant}"
        subprocess.run(cmd_variant, shell=True)

        # Concatenate sample files
        csv_files_sample = self._get_sorted_csv_files(
            f"_res_to_join_{self.chunk_index}_x_sample.csv"
        )
...
def concatenate_samples(file_paths: List[str]) -> pd.DataFrame:
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        for sample_data_chunk in executor.map(read_sample_data, list(file_paths)):
            sample_data.append(sample_data_chunk)

    concatenated_sample_df = pd.concat(sample_data, ignore_index=True)
    aggregate_df = (
        concatenated_sample_df.groupby("SampleID")
        .agg(
            {
                "Accuracy": "sum",
                "N_var": "sum",
    ...
    aggregate_df["Accuracy"] = np.round(
        aggregate_df["Accuracy"] / aggregate_df["N_var"], decimals=6
    )
    aggregate_df["r2"] = np.round(
        np.divide(aggregate_df["r2_per_sample_num"], aggregate_df["r2_per_sample_den"])
        ** 2,
        decimals=6,
    )
    aggregate_df["Precision"] = np.round(
        aggregate_df["TP"] / (aggregate_df["TP"] + aggregate_df["FP"]), decimals=6
    )
    aggregate_df["Recall"] = np.round(
        aggregate_df["TP"] / (aggregate_df["TP"] + aggregate_df["FN"]), decimals=6
    )
    aggregate_df["F1-score"] = np.round(
        2
        * aggregate_df["Precision"]
        * aggregate_df["Recall"]
        / (aggregate_df["Precision"] + aggregate_df["Recall"]),
        decimals=6,
    )
    ...
    aggregate_df.drop(
        ["N_var", "r2_per_sample_num", "r2_per_sample_den"], axis=1, inplace=True
    )
...
def calculate_metrics(
    ...
    with warnings.catch_warnings():
        ...
        vID_imputed, Haps_imputed, sID_imputed = imputed._ingest_data(
            imputed.filepath, chrom, start, end
        )
...
def main():
    parser = argparse.ArgumentParser(
        usage=(
            "%(prog)s --imputed <imputed_file> --wgs <whole_genome_file>\n"
            "Use -h or --help to display help."
        )
    )
    parser.add_argument(
        "--wgs", dest="wgs", type=str, required=True, help="path to whole genome file"
    )
    parser.add_argument(
        "--imputed",
        dest="imputed",
    ...
    parser.add_argument(
        ...
        required=False,
        default="",
        help="File of sample names to include in the analysis. One sample per line.",
    )
    parser.add_argument(
        "-c",
        dest="chunk_size",
        ...
        required=False,
        default=10000,
        help="size of data chunks, [ default: 10000 ]",
    )
    parser.add_argument(
        "--threads",
        dest="threads",
        ...
        required=False,
        default=10,
        help="threads to use for parallel processing, [ default: 10 ]",
    )
    parser.add_argument(
        "--keep_tmp",
        dest="keep_tmp",
    ...
    variant_file_paths = nats.natsorted(
        [variant_file for _, variant_file in file_paths]
    )
