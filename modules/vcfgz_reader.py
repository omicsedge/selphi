...
class vcfgz_reader(object):
    ...
    def __init__(self, ref_panel_path: str):
        ...
        cache_line_copy = create_string_buffer(
            b"\x00" * CACHE_LINE_LEN
        )  # allocate string of NUL characters
    ...
    def read_columns(
        ...
        GENOTYPES: Union[
            np.ndarray, bool
        ] = False,  # a 2d array, cuz may be several columns
    ):
        """
        ...
        if lines_n < 0:
            raise ValueError(
                f"lines_n denotes the number of lines to read, and therefore has to be a non-negative integer"
            )

        metadata_cols_names: List[str] = [
            "#CHROM",
            "POS",
            "ID",
            "REF",
        ...
        metadata_cols_input: List[Union[np.ndarray, bool]] = [
            CHROM,
            POS,
            ID,
            REF,
        ...
        for i, (col_i_name, col_i) in enumerate(
            zip(metadata_cols_names, metadata_cols_input)
        ):
            ...
            if col_i is False:
                ...
                col_i_: np.ndarray = np.char.array(
                    np.zeros(lines_n, dtype=dtype)
                )  # type:ignore # char *is* a known member of np
                ...
                if tuple(col_i.shape) != (lines_n,):
                    raise ValueError(
                        f"passed array-to-refill for {col_i_name} is of wrong shape: {col_i.shape}. Expected shape: (lines_n,) == {(lines_n, )}"
                    )
                if not (
                    col_i.dtype.type == np.bytes_
                    and col_i.dtype.itemsize == COLS_LEN[i]
                ):
                    raise ValueError(
                        f"passed array-to-refill for {col_i_name} is of wrong dtype: {col_i.dtype}. Expected type: {dtype}"
                    )
        ...
        for i, (col_i_name, col_i) in [(0, ("GENOTYPES", GENOTYPES))]:
            if col_i is False:
                ...
                genotype_table_c = (
                    c_int8_2d_array_type()
                )  # initializes with int values = 0
                genotype_table_py = np.ctypeslib.as_array(genotype_table_c)  # type: ignore
            else:  # col_i is np.ndarray
                if tuple(col_i.shape) != (lines_n, self.total_haploids_n):
                    raise ValueError(
                        f"passed array-to-refill for {col_i_name} is of wrong shape: {col_i.shape}. Expected shape: (lines_n, self.total_haploids_n) == {(lines_n, self.total_haploids_n)}"
                    )
                if col_i.dtype.type != data_np_type:
                    raise ValueError(
                        f"passed array-to-refill for {col_i_name} is of wrong dtype: {col_i.dtype.type}. Expected type: {data_np_type}"
                    )
        ...
        if lines_n > 0:
            ...
            table_row_i = readcolumns_wrapper(
                self.obj,
                lines_n,
        ...
        return lines_read_n, [col for col in metadata_cols_py if not col is False] + [
            gntp_tbl for gntp_tbl in [genotype_table_py] if not gntp_tbl is None
        ]
    ...
    def read_all_lines(
        ...
        if batch_size_lines <= 0:
            raise ValueError(
                f"batch_size_lines denotes the number of lines to read at a time, and therefore has to be a positive integer"
            )

        cols_input: List[bool] = [
            CHROM,
            POS,
            ID,
            REF,
        ...
        cols_batches: List[List[np.ndarray]] = [
            list() for col in cols_input if col is True
        ]
        ...
        if n_batches > 1:
            metadata_cols_full = [
                np.concatenate(col_batches, axis=0) for col_batches in cols_batches
            ]
        else:
    ...
    def readlines(
        ...
        if lines_n <= 0:
            raise ValueError(
                f"lines_n denotes the number of lines to read, and therefore has to be a positive integer"
            )
        ...
        if refill is not None:
            if tuple(refill.shape) != (lines_n, self.total_haploids_n):
                raise ValueError(
                    f"passed array to refill is of wrong shape: {refill.shape}. Expected shape: (lines_n, self.total_haploids_n) == {(lines_n, self.total_haploids_n)}"
                )
            if refill.dtype.type != data_np_type:
                raise ValueError(
                    f"passed array to refill is of wrong dtype: {refill.dtype.type}. Expected type: {data_np_type}"
                )
            array = np.ctypeslib.as_ctypes(refill)  # type: ignore
        else:
            array = c_int8_2d_array_type()  # initializes with int values = 0

        table_row_i: int = self.lib.readlines(
            self.obj, array, c_lines_n, c_total_haploids_n
