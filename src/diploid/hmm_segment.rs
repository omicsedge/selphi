...
impl SegmentHmm {
    ...
    /// Initialize at a homozygous site.
    /// All hap configs get the same probability (allele is the same for all 8).
    fn init_hom(&mut self, target_allele: bool, cond_alleles: &[bool]) {
        self.prob_sum_h = [0.0; HAP_NUMBER];
        for k in 0..self.n_cond {
            let emit = if cond_alleles[k] == target_allele { 1.0f32 } else { MISMATCH };
    ...
    /// Initialize at a heterozygous/ambiguous site.
    /// Different hap configs get different emissions based on ambiguous code.
    fn init_amb(&mut self, amb_code: u8, cond_alleles: &[bool]) {
        // g0[h] = emission if cond carries REF; g1[h] = emission if cond carries ALT
    ...
    fn run_hom(&mut self, target_allele: bool, cond_alleles: &[bool], nt: f32, yt: f32) -> bool {
        ...
        unsafe {
            ...
            let _nt = _mm256_set1_ps(nt_div);
            let _factor = _mm256_set1_ps(yt_div);
    ...
    fn run_amb(&mut self, amb_code: u8, cond_alleles: &[bool], nt: f32, yt: f32) {
        ...
        unsafe {
            ...
            let mut _tfreq = _mm256_loadu_ps(self.prob_sum_h.as_ptr());
            _tfreq = _mm256_mul_ps(_tfreq, _factor);
            let _g0 = _mm256_loadu_ps(g0.as_ptr());
            let _g1 = _mm256_loadu_ps(g1.as_ptr());
            let mut _sum = _mm256_setzero_ps();

            let prob = &mut self.prob;
            for k in 0..self.n_cond {
                let _g = if cond_alleles[k] { _g1 } else { _g0 };
                let base = k * HAP_NUMBER;
                let mut _prob = _mm256_load_ps(prob[base..].as_ptr());
                _prob = _mm256_fmadd_ps(_prob, _nt, _tfreq);
                _prob = _mm256_mul_ps(_prob, _g);
                _sum = _mm256_add_ps(_sum, _prob);
                _mm256_store_ps(prob[base..].as_mut_ptr(), _prob);
            }
    ...
    pub fn forward<F>(
        ...
        let dummy = super::params::HmmParams { ne: 15000.0, n_haps: 0, trans: vec![], cm_f32: vec![], rare_allele: vec![] };
        self.forward_impl(graph, cond_haps, haplotypes, trans, seg_first, seg_last, &[], &dummy);
    }
    ...
    pub fn forward_rare<F>(
        ...
        self.forward_impl(graph, cond_haps, haplotypes, trans, seg_first, seg_last, rare_allele, hmm_params);
    }
    ...
    fn forward_impl<F>(
        ...
        let n_cond = cond_haps.len();
        assert_eq!(n_cond, self.n_cond);

        // Pre-allocate alpha storage for all segments in window
        let n_window_segs = seg_last - seg_first + 1;
        let prob_size = n_cond * HAP_NUMBER;
        // Reuse alpha storage across windows (resize instead of re-allocate)
        self.alpha_store.resize(n_window_segs, Vec::new());
        ...
        // Count ambiguous sites before seg_first
        for s in 0..seg_first {
            let start = graph.segment_start(s);
            for vrel in 0..graph.lengths[s] as usize {
                let vi = start + vrel;
        ...
        let mut prev_abs_locus = abs_locus;
        let mut cond_alleles = vec![false; n_cond]; // pre-allocate once
        ...
        for seg in seg_first..=seg_last {
            ...
            for vrel in 0..seg_len {
                ...
                // Get conditioning haplotype alleles at this locus (reuse buffer)
                for (k, &h) in cond_haps.iter().enumerate() {
                ...
                if var_is_hom(e, byte) {
                    ...
                    let skip_rare = rare >= 0 && (target_allele as i8) != rare;
                    if skip_rare {
                        // Don't update HMM state or prev_abs_locus
                    } else if is_first_seg && is_first_in_seg {
                        self.init_hom(target_allele, &cond_alleles);
                        prev_abs_locus = vi;
                    } else if is_first_in_seg {
                        let (nt, yt) = self.transition_params_full(vi, prev_abs_locus, trans, &hmm_params.cm_f32, hmm_params.ne, hmm_params.n_haps);
                        self.collapse_hom(target_allele, &cond_alleles, nt, yt);
                        prev_abs_locus = vi;
                    } else {
                        let (nt, yt) = self.transition_params_full(vi, prev_abs_locus, trans, &hmm_params.cm_f32, hmm_params.ne, hmm_params.n_haps);
                        self.run_hom(target_allele, &cond_alleles, nt, yt);
                        prev_abs_locus = vi;
                    }
                } else if var_is_het(e, byte) || var_is_sca(e, byte) {
                    let amb_code = graph.ambiguous[abs_ambiguous];
                    if is_first_seg && is_first_in_seg {
                        self.init_amb(amb_code, &cond_alleles);
                    } else if is_first_in_seg {
                        let (nt, yt) = self.transition_params_full(vi, prev_abs_locus, trans, &hmm_params.cm_f32, hmm_params.ne, hmm_params.n_haps);
                        self.sum_k();
                        self.collapse_amb(amb_code, &cond_alleles, nt, yt);
                    } else {
                        let (nt, yt) = self.transition_params_full(vi, prev_abs_locus, trans, &hmm_params.cm_f32, hmm_params.ne, hmm_params.n_haps);
                        self.run_amb(amb_code, &cond_alleles, nt, yt);
                    }
                    abs_ambiguous += 1;
                    prev_abs_locus = vi;
                } else if var_is_mis(e, byte) {
            ...
            // Save alpha at segment boundary
            self.sum_k();
            self.save_alpha(seg - seg_first, abs_locus - 1);

            // Compute transition probabilities for this segment boundary
            if seg < seg_last {
                self.compute_h_probs();
            }
        }
    ...
    where F: Fn(usize, usize) -> bool {
        let dummy = super::params::HmmParams { ne: 15000.0, n_haps: 0, trans: vec![], cm_f32: vec![], rare_allele: vec![] };
        self.backward_impl(graph, cond_haps, haplotypes, trans, seg_first, seg_last, &[], &dummy)
    }
    ...
    where F: Fn(usize, usize) -> bool {
        self.backward_impl(graph, cond_haps, haplotypes, trans, seg_first, seg_last, rare_allele, hmm_params)
    }
    ...
    where F: Fn(usize, usize) -> bool {
        ...
        // Count transitions: dc(seg_first) + boundary transitions
        let dc0 = graph.count_diplotypes(seg_first);
        let mut n_boundary = 0usize;
        for s in seg_first..seg_last {
            n_boundary += graph.count_diplotypes(s) * graph.count_diplotypes(s + 1);
        ...
        // Count ambiguous sites up to seg_last end
        let mut abs_ambiguous_end = 0usize;
        for s in 0..=seg_last {
            let start = graph.segment_start(s);
            for vrel in 0..graph.lengths[s] as usize {
        ...
        // Transition prob write position (fill from end to start)
        let mut trans_write_offset = n_trans;

        // Traverse backward through all loci
        let mut first_locus = true;
        let total_loci = locus_last - locus_first + 1;
        ...
        for locus_idx in 0..total_loci {
            ...
            // Get conditioning alleles (reuse buffer)
            for (k, &h) in cond_haps.iter().enumerate() {
                cond_alleles[k] = haplotypes(vi, h);
            }
            ...
            if first_locus {
                // Initialize at last locus
                if is_hom {
                    self.init_hom(var_get_hap0(e, byte), &cond_alleles);
                } else if is_amb {
                    self.init_amb(graph.ambiguous[abs_ambiguous], &cond_alleles);
                } else {
                    self.init_mis();
                }
            } else if is_first_in_seg && curr_seg < seg_last {
                ...
                if !hap_underflow {
                    let out = &mut transition_probs[trans_write_offset..trans_write_offset + n_t];
                    let dip_underflow = self.compute_trans_dip_mult(
                        &prev_codes, &next_codes, out);
                    let sum_d = if dip_underflow {
                        if self.compute_trans_dip_add(&prev_codes, &next_codes, out) {
                            // Both underflow — leave defaults
                            0.0
                        } else {
                            out.iter().sum::<f64>()
                        }
                ...
                if is_hom {
                    ...
                    self.collapse_amb(graph.ambiguous[abs_ambiguous], &cond_alleles, nt, yt);
                } else {
                    self.collapse_mis(nt, yt);
                }
            } else {
                ...
                if is_hom && rare >= 0 && (var_get_hap0(e, byte) as i8) != rare {
                    ...
                    self.run_hom(var_get_hap0(e, byte), &cond_alleles, nt, yt);
                } else if is_amb {
                    self.run_amb(graph.ambiguous[abs_ambiguous], &cond_alleles, nt, yt);
                } else {
                    self.run_mis(nt, yt);
                }
            }
            ...
            // Track segment position (going backward)
            if curr_seg_locus == 0 && curr_seg > seg_first {
                curr_seg -= 1;
                curr_seg_locus = graph.lengths[curr_seg] as usize - 1;
            } else {
        ...
        if trans_write_offset > 0 && self.prob_sum_t > 0.0 {
            ...
            for (t, &d) in first_codes.iter().enumerate() {
                ...
                if t < trans_write_offset {
                    transition_probs[t] = p;
                    sum_dip += p;
                }
            }
            if sum_dip > 0.0 {
                let inv = 1.0 / sum_dip;
                for t in 0..n_first.min(trans_write_offset) {
                    transition_probs[t] *= inv;
                }
            }
        }
...
mod tests {
    ...
    #[test]
    fn test_init_hom() {
        let mut hmm = SegmentHmm::new(4);
        let cond = vec![true, false, true, false];
        hmm.init_hom(true, &cond);
        assert!(hmm.prob_sum_t > 0.0);
        // Matching cond haps should have prob 1.0, mismatching MISMATCH
        assert!((hmm.prob[0] - 1.0).abs() < 1e-6); // k=0 matches
        assert!((hmm.prob[HAP_NUMBER] - MISMATCH).abs() < 1e-6); // k=1 mismatches
    }
    ...
    fn test_run_hom() {
        ...
        let cond = vec![true, false, true, false];
        hmm.init_hom(true, &cond);
        let sum_before = hmm.prob_sum_t;
        hmm.run_hom(true, &cond, 0.99, 0.01);
        // After transition, sum should still be positive
        assert!(hmm.prob_sum_t > 0.0);
    }
    ...
    fn test_init_amb() {
        let mut hmm = SegmentHmm::new(4);
        // amb_code: hap 0 and 2 carry ALT
        let mut amb = 0u8;
        hap_set(&mut amb, 0);
        hap_set(&mut amb, 2);
        let cond = vec![true, false, true, false];
        hmm.init_amb(amb, &cond);
        assert!(hmm.prob_sum_t > 0.0);
    }
}
