    ...
    public:
        ...
        int readlines(int n){
            ...
            try {
                ...
                for (i = 0; i < n; i++){
                    this->line_v = readline(this->ref_panel);
                    seg[i] = get_haploid_from_line(this->line_v, 6);
                    // if (i == 0){
                    //     cout_vect(v);
                    // }
        ...
        int readlines2(uint8_t* segm, int n, int haploid){
            ...
            try {
                ...
                for (i = 0; i < n; i++){
                    segm[i] = get_haploid_from_line(this->line_v, haploid);
                    this->table_row_i++;
                    this->line_v = readline(this->ref_panel);
        ...
        int readlines3(
            ...
            try {
                ...
                for (i = 0; i < n; i++){
                    ...
                    //     std::cout << "looping at i = " << i << std::endl;
                    // }
                    populate_array_from_line(table, this->line_v, total_haploids, i);
                    this->table_row_i++;
                    this->line_v = readline(this->ref_panel);
                }
            } catch (gzReadPastEOF& e) {
        ...
        int readlines3_int(
            ...
            try {
                ...
                std::cout << "IBD_extractor::readlines3_int():    going to populate the array at " << table << std::endl;
                for (i = 0; i < n; i++){
                    // if (i == 10 || i == 100 || i == 1000 || i == 10000 || i % 100000 == 0)
                    //     std::cout << "looping at i = " << i << std::endl;
                    // if (i >= N_FULL_SEQ_VARIANTS-1){
                    ...
                    //     std::cout << "looping at i = " << i << std::endl;
                    // }
                    populate_array_from_line__int(table, this->line_v, total_haploids, i);
                    this->table_row_i++;
                    this->line_v = readline(this->ref_panel);
                }
            } catch (gzReadPastEOF& e) {
        ...
        int readlines3_selfcontained(
            ...
            try {
                ...
                std::cout << "IBD_extractor::readlines3_selfcontained():    going to populate the array at " << table << std::endl;
                for (line_i = 0; line_i < n; line_i++){
                    line_array = &this->line_v[0];
                    metadata = 0;
                    ...
                    z = line_i*total_haploids;

                    for (int i = 0; i < total_haploids; i++){
                        *(table + z + i) = static_cast<uint8_t>(line_array[metadata + (i * 2)] - '0');
                    }

                    // populate_array_from_line(table, this->line_v, total_haploids, line_i);
                    this->table_row_i++;
                    this->line_v = readline(this->ref_panel);
                }
            } catch (gzReadPastEOF& e) {
...
extern "C" {
    vcfgz_reader* IBDs_extractor_new(const char* path){
        // std::cout << "input path: " << path << std::endl;
        return new vcfgz_reader(path);
    }

    int readlines(vcfgz_reader *ibds_extractor, int lines_n){
        return ibds_extractor->readlines(lines_n);
    }

    int readlines2(vcfgz_reader *ibds_extractor, uint8_t* segm, int lines_n, int haploid){
        return ibds_extractor->readlines2(segm, lines_n, haploid);
    }

    int readlines3(vcfgz_reader *ibds_extractor, uint8_t* table, long long int lines_n, long long int total_haploids){
        return ibds_extractor->readlines3(table, lines_n, total_haploids);
    }

    int readlines3_int(vcfgz_reader *ibds_extractor, uint8_t* table, int lines_n, int total_haploids){
        return ibds_extractor->readlines3_int(table, lines_n, total_haploids);
    }

    int readlines3_selfcontained(vcfgz_reader *ibds_extractor, uint8_t* table, long long int lines_n, long long int total_haploids){
        return ibds_extractor->readlines3_selfcontained(table, lines_n, total_haploids);
    }

    void close_(vcfgz_reader *ibds_extractor){
...
int main(){
    ...
    vcfgz_reader f = vcfgz_reader(REF_PANEL);

    // long long int lines_n = 1647105;
    long long int lines_n = 1647103;
    long long int total_haploids_n = 6404;
    ...
    // long long int z = 300000LLU * 6404LLU;
