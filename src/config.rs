...
static KNOBS: &[Knob] = &[
    ...
    Knob { name: "LCWGS_KEEP_SUPPLEMENTARY", group: "io", kind: Kind::Bool, default_repr: "false", user_facing: false, doc: "Native BAM/CRAM pileup: KEEP supplementary alignments (flag 0x800). Default off = drop them, which deviates from BOTH references rather than matching either: bcftools mpileup's --ff default excludes only UNMAP,SECONDARY,QCFAIL,DUP, and GLIMPSE2 excludes only UNMAP|SECONDARY (its supplementary exclusion is commented out, caller_initialise.cpp:196-199). UNMEASURED." },
    ...
    Knob { name: "SELPHI_DIPLOID_RARE_DISPATCH", group: "experimental", kind: Kind::Bool, default_repr: "false", user_facing: false, doc: "DEPRECATED, now the default and inverted: use SELPHI_DIPLOID_SKIP_FIRST_DISPATCH=1 to get the OLD skip-first order back. Position-first dispatch in both diploid segment-HMM forward passes -- INIT (window head) and COLLAPSE (segment head) run unconditionally and the rare-hom skip applies only in the RUN position, matching SHAPEIT5 haplotype_segment_single.cpp:105-117 and Selphi's own backward pass. Default-on since 2026-09-03. The earlier note that the shadowed-INIT case 'degrades to the f64 fallback' was WRONG: the f64 forward had the same skip-first order and neither has_underflow threshold (1e-30 / 1e-300) is true for NaN, so a poisoned window reported itself healthy and its all-zero transition vector reached all_trans, after which the sampler committed the last diplotype of every affected segment. Both holes are now closed." },
    Knob { name: "SELPHI_DIPLOID_SKIP_FIRST_DISPATCH", group: "experimental", kind: Kind::Bool, default_repr: "false", user_facing: false, doc: "Restore the OLD skip-first dispatch order in both diploid segment-HMM forward passes, where the rare-hom skip is tested before the position so a skip on a window or segment head shadows INIT/COLLAPSE. Position-first (SHAPEIT5 haplotype_segment_single.cpp:105-117, and what this file's own backward pass does) is the default since 2026-09-03. Byte-identical on chr22-801 (the rare path is thin there); kept only as an A/B escape hatch." },
    Knob { name: "SELPHI_HMM_NO_XWIN_PRIOR", group: "experimental", kind: Kind::Bool, default_repr: "false", user_facing: false, doc: "Skip the cross-window forward prior entirely -- neither build nor consume it. Measured INERT on chr1 (801 samples, 5,769,087 variants, 4 real windows, 1KG panel): output is byte-identical with the true posterior, with the old boundary-set indicator, and with no prior at all, because finalize_weights rewrites rows 0-1 and n-2/n-1 of every window and the first overlap/2 markers are discarded. Skipping it frees in_flight_haps x n_states x 16 bytes of held memory plus the time to build it. Not yet the default: verified on ONE panel scale only." },
    Knob { name: "SELPHI_HMM_XWIN_BOUNDARY_PRIOR", group: "experimental", kind: Kind::Bool, default_repr: "false", user_facing: false, doc: "Restore the OLD cross-window forward prior, which was read AFTER the last block's boundary fill had replaced the final forward row with 1/nh at the matched states and 0 elsewhere -- a match-set indicator that had forgotten the accumulated forward evidence. The default now reads the row before that fill, so each window hands the next one the real posterior. The fill is harmless to the weights (finalize_weights rewrites rows n-2 and n-1 of every window), which is why this went unnoticed. Affects multi-window chromosomes only: chr21 and chr22 are single-window and never build the prior." },
    Knob { name: "SELPHI_HMM_TOTAL_RECOMB_DENOM", group: "experimental", kind: Kind::Bool, default_repr: "false", user_facing: false, doc: "Imputation HMM: divide the per-site recombination shift by the summed group size over ALL states instead of over the haplotypes MATCHED at that site, which is what makes the kernel mass-preserving (the shift is added over all n_states either way, so the shipped divisor inflates the injected mass by the all-states/matched ratio). NOT the default: the shipped auto-Ne (36.4 * n_ref, floor 100,000) was swept on top of the current kernel, so the inflation is absorbed into that constant and moving one without re-sweeping the other makes the copying model far stickier than anything validated. A one-parameter A/B at fixed Ne is not a verdict." },
    Knob { name: "SELPHI_DIPLOID_DISPATCH_DIAG", group: "debug", kind: Kind::Bool, default_repr: "false", user_facing: false, doc: "Count diploid forward-pass rare-hom skips that land on a window head or a segment head. NOTE these counters are STRUCTURALLY PINNED TO ZERO and prove nothing: GenotypeGraph::build only breaks a segment at a locus whose own het/sca flag reaches predicted_unfold == 4, so every segment head after the first is HET or SCA and the var_is_hom-gated skip chain can never reach a segment head; the window-head case needs graph variant 0 itself to be panel-rare. A zero reading here is a tautology, not evidence -- to measure skip volume, use SELPHI_DIPLOID_ALPHA_DIAG, which counts skips at a segment's LAST locus (that is the case save_alpha was mislabelling)." },
    Knob { name: "SELPHI_HMM_TL_KEEP_MAX", group: "experimental", kind: Kind::Bool, default_repr: "false", user_facing: false, doc: "Restore the pre-2026-09-04 thread-local scratch policy in the chip/WGS HMM: keep the forward and weight buffers at the largest size any haplotype ever needed, for the life of the rayon worker. Those buffers are n_window_sites * n_states, and n_states is per-TARGET (MESA 100 x TOPMed chr20: mean 2,276, max ~18,500), so a worker kept the largest hap it ever saw resident for the rest of the run. The default now drops a retained buffer more than 2x larger than the current hap needs and allocates a right-sized one instead. MEASURED worth 0.82 GB of a 42.98 GB peak (42.16 vs 42.98, byte-identical output, wall unchanged) -- real and free, but small: the peak is the LIVE working set of one hap per thread (43.3 GB at 16 threads vs 21.9 GB at 4 on the same rig), not retention. Set 1 only to A/B the memory or to chase an allocator regression." },
    Knob { name: "SELPHI_DIPLOID_ALPHA_DIAG", group: "debug", kind: Kind::Bool, default_repr: "false", user_facing: false, doc: "Measure the save_alpha locus-label defect fixed 2026-09-03. Both labels are stored per segment -- the locus that last UPDATED the state (shipped) and the segment's LAST locus (pre-fix) -- and this counts how often they differ, the gap in loci and in cM, and, at the segment boundary where compute_trans_hap consumes it, the ratio between the recombination term yt each label produces. Unlike SELPHI_DIPLOID_DISPATCH_DIAG these counters are NOT structurally pinned to zero: they fire on a rare-hom skip anywhere inside a segment, not only on its head. The rare path needs panel-MAF < 0.001 sites in the phasing set, so a chip x small-panel run reads zero and a chip x TOPMed (n_haps_total ~171k, MAF < 0.001 = AC < 171) run does not. Aggregated over samples x iterations x windows and over both precision twins; reset + printed per phasing run. is_one(), OnceLock-cached (diploid/alpha_diag.rs; reported from diploid/phase_common.rs)." },
    Knob { name: "SELPHI_DIPLOID_ALPHA_SEG_END", group: "experimental", kind: Kind::Bool, default_repr: "false", user_facing: false, doc: "Restore the pre-2026-09-03 Alpha locus label: compute_trans_hap bridges from the segment's LAST locus instead of the last locus that actually updated the state. Under a rare-hom skip at a segment boundary the old label makes the bridge too short, yt too small and the phase too sticky; SHAPEIT5 and this file's own backward pass both use the updated locus. This is the exact A/B for the fix -- paired runs differ only where the rare path fires, and are byte-identical wherever it does not. Pair it with SELPHI_DIPLOID_ALPHA_DIAG to check the rig exercises the path at all before reading a delta. is_one(), OnceLock-cached (diploid/alpha_diag.rs; consumed by both segment-HMM twins)." },
