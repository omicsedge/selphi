...
static KNOBS: &[Knob] = &[
    ...
    Knob { name: "SELPHI_HAPLOID_NOCALL_LOCAL", group: "experimental", kind: Kind::Bool, default_repr: "false", user_facing: true, doc: "Impute no-call (missing) target genotypes in the init PBWT phaser from the LOCAL copying consensus (majority over the K nearest PBWT neighbours) instead of the Beagle-faithful random draw weighted by the GLOBAL allele CDF. Closes the missing-genotype phasing gap vs Beagle on real arrays (~1-2% no-calls) at 1× cost. Fires only when a target genotype is missing → byte-identical on no-missing input. is_one() (haploid/pbwt.rs phase_subwindow). Default off." },
    Knob { name: "SELPHI_HAPLOID_NOCALL_K", group: "experimental", kind: Kind::Value, default_repr: "50", user_facing: false, doc: "Number of nearest PBWT neighbours used by the SELPHI_HAPLOID_NOCALL_LOCAL majority vote (expands outward, skipping missing neighbours). Swept on v3 chr22: K=50 optimal (+0.0024 SNP), K=20 too noisy (+0.0007), K=100 dilutes (+0.0011). usize_or(50). haploid/pbwt.rs phase_subwindow." },
