...
static KNOBS: &[Knob] = &[
    ...
    Knob { name: "SELPHI_DIPLOID_MAIN_SAMPLE", group: "debug", kind: Kind::Value, default_repr: "", user_facing: false, doc: "Intra-run phase-ensemble PROTOTYPE (Route A): commit the m-th Main MCMC sample as the diploid phase instead of the Viterbi solve. Averaging imputations over m=0..K-1 at the same --seed marginalizes phase uncertainty from a SINGLE phasing chain (the Main samples are free posterior draws otherwise discarded). Unset (default) = Viterbi solve, byte-identical. raw().parse::<usize>() (diploid/mod.rs _diploid_run)." },
