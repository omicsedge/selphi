...
/// Bitmatrix-backed deduplication: identical algorithm to
/// `deduplicate_haplotypes` but reads allele bits from the reference bitmatrix
/// instead of a materialized `n_chip × n_haps` byte array. Saves the dense
/// `ref_w` allocation per window (up to hundreds of MB at biobank scale).
