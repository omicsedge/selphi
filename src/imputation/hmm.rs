...
/// Convert CSC match matrix to per-site match lists with match expansion.
///
/// The CSC matrix stores matches at their divergence point with length data.
/// A match at column `v` with length `L` for hap `h` means hap `h` matches
/// from variant `max(0, v - L + 1)` through variant `v`.
/// We expand each match into coverage at ALL variant positions in the range.
///
/// Returns expanded per-site match lists (no scores needed after expansion).
fn csc_to_per_site_matches(csc: &CscMatchMatrix) -> (Vec<Vec<i64>>, Vec<Vec<i32>>) {
    let n_var = csc.n_cols;
    let n_ref = csc.n_rows;

    // First pass: expand matches and accumulate per-site hap sets
    // Use a vec of sets for deduplication
    let mut site_haps: Vec<Vec<i64>> = vec![Vec::new(); n_var];
    let _hap_seen: Vec<Vec<bool>> = Vec::new(); // We'll use a flat array instead

    // Use a flat bitmap for efficient dedup: (n_var) bits per hap
    // Actually simpler: for each match, mark the hap present at each covered site
    let _present = vec![false; n_ref]; // reusable per-site dedup

    // Approach: iterate over all match entries, expand ranges
    for v in 0..n_var {
        let col_start = csc.indptr[v] as usize;
        let col_end = csc.indptr[v + 1] as usize;
        for k in col_start..col_end {
            let hap = csc.indices[k] as usize;
            let length = csc.data[k] as usize;
            // Match covers from (v - length + 1) through v (inclusive)
            // But the divergence semantics: match starts at d[pos] and extends to v
            let range_start = v.saturating_sub(length - 1);
            let range_end = v; // inclusive
            for site in range_start..=range_end {
                if site < n_var {
                    site_haps[site].push(hap as i64);
                }
            }
        }
    }

    // Deduplicate per site (keep unique haps)
    for site in 0..n_var {
        site_haps[site].sort_unstable();
        site_haps[site].dedup();
    }

    // Dummy scores (not needed after expansion)
    let scores = vec![Vec::new(); n_var];
    (site_haps, scores)
}
...
/// Backward pass of the Li-Stephens HMM.
fn compute_backward(
    dense_matches: &[Vec<usize>],
    n_matches: &[usize],
    n_haps: &[f64],
    ...
    let p_no_err = 1.0 - p_err;
    let n_rows = stop - start;
    let mut bwd = vec![0.0f64; n_rows * n_states];
    let use_weighted = emission_ratios.is_some();

    // Initialize last row
    let last_base = (n_rows - 1) * n_states;
    // Initialize last row: ALL states uniform (matches Python)
    if let Some(beta) = init_beta {
        bwd[last_base..last_base + n_states].copy_from_slice(beta);
    } else {
        let last_chip = (start + n_rows - 1).min(n_haps.len() - 1);
        let nh = n_haps[last_chip];
        let val = 1.0 / nh;
        for j in 0..n_states {
            bwd[last_base + j] = val;
        }
    ...
    for row in (0..n_rows - 1).rev() {
        ...
        if chip_idx >= r_precomb.len() { continue; }
        let nh = n_haps[chip_idx];
        let nm = n_matches[chip_idx];
        let p_rec = r_precomb[chip_idx];

        let scale = (1.0 - p_rec * nh) / last_sum;
        let next_base = (row + 1) * n_states;
        let cur_base = row * n_states;
        ...
        if let Some(gs) = group_sizes {
            for j in 0..n_states {
                let val = scale * bwd[next_base + j] + p_rec * gs[j];
                bwd[cur_base + j] = p_err * val;
                let y = bwd[cur_base + j] - kahan_c;
                let t = row_sum + y;
                kahan_c = (t - row_sum) - y;
                row_sum = t;
            }
            ...
            let shift = p_rec;
            for j in 0..n_states {
                let val = scale * bwd[next_base + j] + shift;
                bwd[cur_base + j] = p_err * val;
                let y = bwd[cur_base + j] - kahan_c;
                let t = row_sum + y;
                kahan_c = (t - row_sum) - y;
        ...
        if use_weighted {
            ...
            for k in 0..nm {
                let j = dense_matches[chip_idx][k];
                let old = bwd[cur_base + j];
                let new_val = old * ratios[chip_idx][k];
                bwd[cur_base + j] = new_val;
                let y = (new_val - old) - kahan_c;
                let t = row_sum + y;
                kahan_c = (t - row_sum) - y;
                row_sum = t;
            }
        } else {
            let ratio = p_no_err / p_err;
            for k in 0..nm {
                let j = dense_matches[chip_idx][k];
                let old = bwd[cur_base + j];
                let new_val = old * ratio;
                bwd[cur_base + j] = new_val;
                let y = (new_val - old) - kahan_c;
                let t = row_sum + y;
                kahan_c = (t - row_sum) - y;
        ...
        if prune_threshold > 0.0 && last_sum > 0.0 {
            let thresh = prune_threshold * last_sum;
            let mut pruned_sum = 0.0;
            for j in 0..n_states {
                if bwd[cur_base + j] >= thresh {
                    pruned_sum += bwd[cur_base + j];
                }
            }
            if pruned_sum > 0.0 {
                let rescale = last_sum / pruned_sum;
                for j in 0..n_states {
                    if bwd[cur_base + j] < thresh {
                        bwd[cur_base + j] = 0.0;
                    } else {
                        bwd[cur_base + j] *= rescale;
                    }
                }
            }
        }
    ...
    let first_beta = bwd[..n_states].to_vec();
    (bwd, first_beta, last_sum)
}
...
/// Combine forward and backward probabilities into normalized weight matrices.
/// Returns a map: breakpoint_start → CsrWeights.
fn combine_fwd_bwd(
    fwd: &[f64],
    bwd: &[f64],
    n_rows: usize,
    ...
    // Element-wise multiplication: weights = fwd * bwd
    let n_total = n_rows * n_states;
    let mut weights = vec![0.0f32; n_total];
    for i in 0..n_total {
        weights[i] = (fwd[i] * bwd[i]) as f32;
    }
    ...
    if is_first_block {
        ...
        let src_base = src * n_states;
        for j in 0..n_states {
            weights[j] = weights[src_base + j]; // row 0
        }
        if n_rows > 1 {
            for j in 0..n_states {
                weights[n_states + j] = weights[src_base + j]; // row 1
            }
        }
    }
    if is_last_block {
    ...
    // Normalize rows + threshold
    let threshold = 1.0 / (n_states as f64 + 1.0);
    for row in 0..n_rows {
        let base = row * n_states;
        let mut row_sum: f64 = weights[base..base + n_states].iter().map(|&v| v as f64).sum();
        ...
        if row_sum == 0.0 {
            ...
            if chip_idx > 0 && chip_idx <= n_matches.len() {
                let ci = chip_idx - 1;
                let nm = n_matches[ci.min(n_matches.len() - 1)];
                let nh = n_haps[ci.min(n_haps.len() - 1)] as f32;
                let dm_ci = ci.min(dense_matches.len() - 1);
                for k in 0..nm {
                    let j = dense_matches[dm_ci][k];
                    weights[base + j] = 1.0f32 / nh;
        ...
        for j in 0..n_states {
            weights[base + j] *= inv_sum;
            if weights[base + j] < threshold_f {
                weights[base + j] = 0.0;
            }
        }
    }

    // Build CSR matrix, expanding dedup groups if needed
    build_csr_from_weights(&weights, n_rows, n_states, n_hid, state_to_hap, group_members)
}
