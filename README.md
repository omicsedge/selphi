...
**Indel sites** are not scored from a single base; instead, when `--reference` is given, Selphi builds the local REF and ALT haplotypes for each panel indel from the FASTA and computes `P(read | REF)` and `P(read | ALT)` for every spanning read with a GATK-style pair-HMM (forward, affine gaps, per-base-quality emissions), folding the two likelihoods into the genotype GLs exactly as a SNP base. Without `--reference` (BAM only) indel sites are left flat and imputed from the panel/LD. On a real 30× WGS sample the per-read indel likelihoods agree with `bcftools mpileup` on 16/16 of matched biallelic indels in indel direction and 13/16 in exact called genotype (the few differences are het-vs-hom-alt calls in homopolymer runs, where indel callers routinely disagree). Tuning knobs: `LCWGS_INDEL_GAP_OPEN`/`LCWGS_INDEL_GAP_EXT` (Phred 45/10), `LCWGS_INDEL_FLANK` (haplotype flank, 25 bp), and an optional homopolymer-aware gap-open (`LCWGS_INDEL_HP_SLOPE`, off by default); set `LCWGS_NO_INDEL_REALIGN=1` to leave indels flat.
