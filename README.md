...
Selphi supports four output formats, selected via command-line flags:

| Flag | Format | Dosage | Best for |
|------|--------|--------|----------|
| *(default)* | VCF.gz | Yes (text, 2 decimal) | Standard bioinformatics, bcftools |
| `--bcf` | BCF 2.2 | Yes (float32, full precision) | Fast downstream parsing, no external deps |
| `--parquet` | Apache Parquet (zstd) | Yes (float32, full precision) | Data science, cloud analytics, Polars/DuckDB |
| `--pgen` | PLINK2 PGEN (.pgen/.pvar/.psam) | Hardcall only (2-bit) | GWAS with plink2 |
...
# PLINK2 PGEN
selphi --refpanel ref.srp --input input.vcf.gz --map map.map --out result --pgen
```
...
For Parquet output, the schema is: `CHROM`, `POS`, `ID`, `REF`, `ALT`, `AF`, `DR2`, `IMP`, then one `DS` column per sample (float32).

For PGEN output, hardcall genotypes are stored as 2-bit packed values. For dosage support, use VCF/BCF output and import into plink2 with `plink2 --vcf result.vcf.gz dosage=DS --make-pgen`.
...
| `--bcf` | Write native BCF output | off |
| `--parquet` | Write Apache Parquet output | off |
| `--pgen` | Write PLINK2 PGEN output | off |
