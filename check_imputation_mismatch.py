...
from typing import OrderedDict as OrderedDictType, Dict, List
import sys
...
# imputed_vcfgz = f'./data/separated_datasets/chr20/reference_panel.30x.hg38_chr20_noinfo.imputed-292-input-samples_ALL.vcf.gz'
# true_vcfgz = f'./data/separated_datasets/chr20/reference_panel.30x.hg38_chr20_noinfo.fullseq-292-input-samples.vcf.gz'
...
try:
    ...
    print(ValueError("""you have to supply 2 paths to 2 vcf/vcf.gz files as arguments"""))
    exit(10)
...
# accuracy_table: OrderedDictType[str, Dict[str, int]] = OrderedDict() # actually counts mismatches
accuracy_table: List[Dict[str, int]] = [] # actually counts mismatches
for sample in imputed.samples:
    accuracy_table.append(dict({
        'hap0': 0,
        'hap1': 0,
        'genotype': 0,
    }))
...
while (lines_read_n1 != 0 and lines_read_n2 != 0):
    lines_read_n1, [imputed_GT] = imputed.read_columns(lines_n=batch_size, GENOTYPES=imputed_GT)
    ...
    # imputed_GT = imputed_GT[:lines_read_n1]
    # true_GT = true_GT[:lines_read_n2]

    assert imputed_GT.shape[1] == true_GT.shape[1], 'number of samples in the two datasets has to be the same'
    assert lines_read_n1 == lines_read_n2, 'number of sites in the two datasets has to be the same'

    # haps_mismatch_batch: np.ndarray = imputed_GT != true_GT
    # imputed_unphased_GT: np.ndarray = imputed_GT[:,::2]+imputed_GT[:,1::2]
    # true_unphased_GT: np.ndarray = true_GT[:,::2]+true_GT[:,1::2]
    # GT_mismatch_batch: np.ndarray = imputed_unphased_GT != true_unphased_GT
